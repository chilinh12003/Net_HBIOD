    ...
    public partial class Ad_ResendMT : System.Web.UI.Page
    {
        ...
        protected void btn_SendMT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mTelco == MyConfig.Telco.Nothing || mTable.Rows.Count < 1)
                {
                ...
                int PartnerID = 0;
                if (mTable.Rows[0]["PartnerID"] == DBNull.Value)
                {
                    int.TryParse(mTable.Rows[0]["PartnerID"].ToString(), out PartnerID);
                }
                if (SendMT(RegKeyword, MSISDN, MTContent))
                {
                    UpdateMOLog(MSISDN, DefineMT.MTType.Default, string.Empty, MTContent,PartnerID);
        ...
        private void UpdateMOLog(string MSISDN, DefineMT.MTType mMTType, string LogContent, string MT, int PartnerID)
        {
            try
            {
